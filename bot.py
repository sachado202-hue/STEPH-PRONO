...
#  CONFIG
# ============================================================
BOT_TOKEN      = "7969169742:AAEp9YoO4IdnTZ4grCVQbkx1m9RWvKoDzQw"
FREE_CHANNEL   = "@stephpronofree"
VIP_CHANNEL_ID = -1003762469696
GROQ_API_KEY   = "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"
SPORT_API_KEY  = "2be493dfb8mshdd67a4a2612c42bp18ba80jsn324f8427bfc0"
...
async def get_todays_matches():
    ...
    endpoints = [
        ...
        },
        {
            ...
            "params": {"date": today, "timezone": "Africa/Dakar"}
        }
    ]
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ep["url"], headers=headers, params=ep["params"],
                    ...
                    if "response" in data:
                        ...
                        for f in data["matches"][:10]:
                            matches.append({
                                "home":   f.get("homeTeam", {}).get("name", "Équipe A"),
                                "away":   f.get("awayTeam", {}).get("name", "Équipe B"),
                                "league": f.get("tournament", {}).get("name", "Championnat"),
                                "time":   f.get("startTimestamp", "")
                            })

                    if matches:
                        logger.info(f"✅ {len(matches)} matchs récupérés via {ep['host']}")
                        return matches
        except Exception as e:
            logger.warning(f"⚠️ {ep['host']}: {e}")
            continue
